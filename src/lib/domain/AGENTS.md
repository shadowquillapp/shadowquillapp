...
| File | Role |
|---|---|
| `presets.ts` (~494 lines) | `Preset`, `parsePreset`, `getPresets`, `getPresetById`, `getDefaultPresets`, `ensureDefaultPreset`, `savePreset`, `deletePresetByIdOrName`, `getPresetHistory`, `rollbackPreset`, `compareVersions`, `exportPresetWithHistory`, `importPresetWithHistory`. **Ships 10 seeded presets.** Versioning = JSON-deep-equal of `options` + `taskType`; keeps `MAX_VERSIONS = 10`. |
...
- **Versioning is JSON-stringify diff.** `savePreset` appends a new version when `JSON.stringify(options) + taskType` changes; rollback rewinds to a target version.
...
- ❌ **Do not store non-serializable values in `Preset.options`** — versioning deep-equals `JSON.stringify` of it.
- ❌ **Do not raise `MAX_VERSIONS`** silently — changing it is a data-migration concern.
