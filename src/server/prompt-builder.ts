...
const PROMPTCRAFTER_SYSTEM_PROMPT = `You are PromptCrafter, an expert at authoring high-performance prompts for AI models.
...
function detectUserIntent(input: string, taskType: TaskType, _options?: GenerationOptions): {
  ...
  const mentionsPrompt = /\b(prompt|system prompt|instruction|ai prompt|prompting)\b/i.test(trimmed);
  const isQuestionLike = /(\?|^\s*(how|can|could|would|should|do you|does|what|why|where|when)\b)/i.test(trimmed);
  const politeRequest = /^\s*(can|could|would|please|help|show)\b/i.test(trimmed);

  const hasTechnicalIntent = technicalKeywords.test(trimmed) ||
                            howToPatterns.test(trimmed) ||
                            technicalTerms.test(trimmed);

  // Check if user is providing actual prompt text to work with
  ...
  if (!looksLikeExistingPrompt && !mentionsPrompt && hasTechnicalIntent && (isQuestionLike || politeRequest) && (taskType === 'coding' || taskType === 'general')) {
    return {
...
function buildOptionDirectives(taskType: TaskType, options?: GenerationOptions): string[] {
  ...
  if (options.detail) {
    const detailMap: Record<'brief' | 'normal' | 'detailed', string> = {
      ...
      detailed: 'Provide rich detail: include background, explicit objectives, scope boundaries, sequential steps, guardrails, edge cases, and validation criteria. Use at least five distinct sentences or bullet lines.'
    };
  ...
  if (taskType === 'image' || taskType === 'video') {
    if (options.stylePreset) {
      directives.push(`Use the ${options.stylePreset} visual style.`);
    }
    if (options.aspectRatio) {
      directives.push(`Target an aspect ratio of ${options.aspectRatio}.`);
    }
  }
  ...
  if (taskType === 'video') {
    ...
    if (typeof options.frameRate === 'number') {
      directives.push(`Assume a frame rate of ${options.frameRate} fps.`);
    }
    if (options.cameraMovement) {
      directives.push(`Favor ${options.cameraMovement} camera movement.`);
    }
    if (options.shotType) {
      directives.push(`Compose primarily as ${options.shotType} shots.`);
    }
  }
