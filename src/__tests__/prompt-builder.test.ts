...
describe("buildUnifiedPromptCore", () => {
	...
	it("should record xml format as a compiler constraint", () => {
		const result = buildUnifiedPromptCore({
			...defaultParams,
			options: { format: "xml" },
		});
		expect(result).toContain("Constraints: format=xml");
	});
...
describe("buildRefinementPromptCore", () => {
	...
	describe("XML format", () => {
		it("should preserve refinement sections without legacy xml wrappers", () => {
			const result = buildRefinementPromptCore({
				...defaultRefinementParams,
				options: { format: "xml" },
			});
			expect(result).toContain("Existing Compiled Prompt:");
			expect(result).not.toContain("<existing_prompt>");
		});
	});
