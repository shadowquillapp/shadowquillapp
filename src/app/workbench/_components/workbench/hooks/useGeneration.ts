...
function openOllamaSettings(): void {
...
async function persistMessage(
	...
	tabManager.pushMessageForTab(tabId, message);
	...
	try {
		...
		if (createdId) {
			tabManager.updateMessageForTab(tabId, message.id, { id: createdId });
			finalId = createdId;
		}
...
export function useGeneration(
	...
	setOutputAnimateKey: Dispatch<SetStateAction<number>>,
) {
	...
	const send = useCallback(
		async (overrideText?: string) => {
			...
			const text = (overrideText ?? activeTab.draft).trim();
			...
			try {
				...
				// Run the model first. Only persist a project, messages, and a
				// version once we actually have output — so a stopped or failed
				// generation leaves nothing behind.
				const output = await callLocalModelClient(prompt, {
					taskType: tabTaskType,
					options,
				...
				await persistMessage(tabManager, tabId, projectId, user);
				...
				const finalAssistantId = await persistMessage(
					tabManager,
					tabId,
					projectId,
					assistant,
				...
				} else if (error?.message === "Model not configured") {
					openOllamaSettings();
					tabManager.setErrorForTab(tabId, null);
				} else {
